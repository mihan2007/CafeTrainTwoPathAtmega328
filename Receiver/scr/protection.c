...
static uint16_t   overloadStartTick = 0;  // старт аварийного сообщения
static uint16_t   testPulseStartTick = 0; // старт короткой проверки A3
static uint8_t    diagnosticPath = 1;     // 1..2 = проверяемый путь
static uint8_t    diagnosticTable = 0;    // 0 = путь без столов, иначе номер стола
